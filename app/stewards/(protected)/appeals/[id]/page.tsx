  ...
  return (
    <div className="space-y-5">
      ...
      <div className="steward-panel relative overflow-hidden rounded-2xl p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-bold uppercase tracking-widest text-steward-gold/60">Appeal</p>
            <h1 className="font-display mt-1 text-2xl font-bold text-steward-cream">
      ...
      <section className="steward-panel rounded-2xl p-5">
        <h2 className="text-base font-semibold uppercase tracking-wider text-steward-gold">1 · Appeal</h2>
        ...
        {(appeal.attachments.length > 0 || appeal.links.length > 0) && (
          <div className="mt-4 border-t border-white/10 pt-4">
            <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-steward-gold/60">Evidence</p>
      ...
      {/* Verdict */}
      <section className="steward-panel rounded-2xl p-5">
        <h2 className="text-base font-semibold uppercase tracking-wider text-steward-gold">2 · Appeal Verdict</h2>
      ...
      {canEditVerdict && !verdict?.is_published && (
        <section className="steward-panel rounded-2xl p-5">
          <h3 className="text-base font-semibold uppercase tracking-wider text-steward-gold">Appeal Status</h3>
          <form action={updateAppealStatusAction} className="mt-3 flex flex-wrap items-center gap-3">
            ...
            <FormActionButton idleLabel="Update Status" loadingLabel="Updating…"
              className="rounded-full border border-white/25 bg-white/8 px-4 py-2 text-sm font-semibold transition hover:border-steward-gold/50 hover:bg-steward-gold/15 hover:text-steward-cream disabled:opacity-50" />
          </form>
        </section>
      )}
      ...
      {canInternal && (
        <section className="steward-panel rounded-2xl border border-[#7020B0]/40 p-5">
          <h3 className="text-base font-semibold uppercase tracking-wider text-steward-gold">3 · Internal Discussion</h3>
      ...
      <div className="pt-2 flex items-center gap-4">
        {originalCase && (
          <Link href={`/stewards/cases/${originalCase.id}`}
            className="text-xs text-white/40 hover:text-steward-gold transition">
            ← Back to original case
          </Link>
        )}
        <Link href="/stewards/appeals" className="text-xs text-white/40 hover:text-steward-gold transition">
          ← All appeals
        </Link>
      </div>
    </div>
          ...
          {attachments.map((a) =>
            isImage(a.url) ? (
              <a key={a.url} href={a.url} target="_blank" rel="noopener noreferrer"
                className="group relative block overflow-hidden rounded-xl border border-steward-gold/25 shadow-md transition hover:border-steward-gold/70">
