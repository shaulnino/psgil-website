...
- **Navigation:** "My Account" is the single header hub for all signed-in areas — Profile, the **Steward module** (nested), and future driver-only areas (attendance). Stewards is no longer a top-level nav link. Guests see "Sign in". Driven by an `authed` prop from the root layout (`components/AccountMenu.tsx`).
