...
- Data modules: `resultsData`, `driversData`, `scheduleData`, `rewardsData`, `newsData`, and the stats engine (`statsComputed.ts` ~1550 lines — now used only server-side for the legacy Drivers metrics table and the Rankings *Ratings* category). The bespoke `h2h.ts` engine has been removed; `statsMetricRegistry.ts` is no longer imported by the UI. All read-only; no writes back to the Sheet.
...
**Profile pattern (`lib/stats/*`).** The Drivers, League, **Circuits**, **Rankings** and **Head-to-Head** tabs build a single normalized dataset once (`normalizeRaces(results, events)` → `NormalizedRace[]`) and compute filter-aware, i18n-ready profiles client-side: `computeDriverProfile`, `computeLeagueProfile`, `computeCircuitProfile(allRaces, events, circuitId, filters)`, `computeLeaderboard(allRaces, filters)`, and `computeH2HProfile(allRaces, driverIdA, driverIdB, filters)`. Metric labels/tooltips are keyed by id via `metricCatalog.ts` / `circuitMetricCatalog.ts` (never hardcoded), and circuit identity is canonicalized in `circuitIdentity.ts`. Records/milestones are always all-time and ignore active filters; grid-to-finish movement and averages exclude DNF/DNS/DSQ; the official (non-reverse) grid drives qualifying metrics. The legacy `computeCircuitStats` in `statsComputed.ts` is now unused by the UI.

**Cross-tab statistical contract.** Every stats tab shares one set of definitions so a number is identical wherever it appears: denominator for rates/averages = **starts** (entries − DNS); a **classified finish** is status `finished` (DNF/DNS/DSQ excluded); grid/pole/avg-grid **exclude reverse-grid** events; points are the **final CSV points** (post-penalty); rate/average metrics are gated by a **minimum sample** (`MIN_SAMPLE`). `lib/stats/leaderboard.ts` derives every Rankings value from `computeDriverProfile` (so a ranked value provably equals that driver's own profile — see `tests/stats/leaderboard.test.ts`) and applies standard competition ranking (1, 2, 2, 4) with min-sample gating; the Rankings *Ratings* category is the one exception, sourced from the scope-based `computeDriverStats` percentile ratings and therefore ignoring the advanced filters. Head-to-Head uses **Model A (shared events only)**: an event counts only when **both drivers started** (a DNS lists the event but excludes it from win/lose), a classified finish beats a DNF/DSQ, and all summary metrics are computed over the shared-start sample. Rankings state lives under `r*`/`format`/`comp`/… URL params; Head-to-Head under `da`/`db`/`h*`.
...
| 2026-07-31 | **Rankings + Head-to-Head rebuilt on the profile pattern; cross-tab statistical contract enforced.** Both tabs moved onto the `lib/stats/*` model and now share one set of definitions with Drivers/League/Circuits (starts denominator; classified = `finished`; reverse-grid excluded from grid/pole; final post-penalty points; `MIN_SAMPLE` gating). New `lib/stats/leaderboard.ts` (`computeLeaderboard` derives every Rankings value from `computeDriverProfile` for provable parity, plus `rankLeaderboard` — standard competition ranking 1/2/2/4 with min-sample gating) and `lib/stats/h2hProfile.ts` (`computeH2HProfile` — Model A shared-events-only: an event counts only when both drivers started, classified beats DNF/DSQ, DNS listed-but-excluded; `listH2HDrivers`). New client modules `components/stats/rankings/{RankingsSection,config}.tsx` (8 categories incl. a scope-only *Ratings* category sourced from `computeDriverStats`, sortable leaderboard, top-3 cards, vs-field bars, below-minimum drivers surfaced separately) and `components/stats/h2h/H2HSection.tsx` (fully Hebrew/RTL, comparison cards from `metricCatalog`, win bar, multi-metric trend chart, shared-event table + results modal). `StatsPageContent.tsx` is now a lean orchestrator (all in-file Rankings/H2H code and their English-hardcoded labels removed). Added optional `team` filter to `ProfileFilters`; added `h2hWins`/`gridWins` to `metricCatalog`; added `rankingsTab.*`, `h2h.chart.*` + labels (en + he); tests `tests/stats/{leaderboard,h2hProfile}.test.ts`. **Deleted the bespoke `lib/h2h.ts` engine** (replaced by `h2hProfile.ts`); `lib/statsMetricRegistry.ts` is now unused by the UI. | Kill the third/ fourth stats engines and the English-only Rankings/H2H strings; guarantee a number means the same thing on every tab, and give Rankings/H2H full Hebrew support + fair, sample-aware comparisons. |
| 2026-07-31 | **Circuits tab rebuilt on the profile pattern.** The Circuits tab moved off the legacy `computeCircuitStats` participation grid onto the same `lib/stats/*` model the Drivers/League tabs use. New `lib/stats/circuitIdentity.ts` (canonical venue id — one venue = one circuit, aliases folded onto a stable slug used as the `?circuitid=` URL key; carries `track_he` + flag), `lib/stats/circuitMetricCatalog.ts` (typed, i18n-keyed metric defs), and `lib/stats/circuitProfile.ts` (`computeCircuitProfile` → snapshot, qualifying-vs-race incl. grid↔finish scatter + winner-grid distribution, movement/reliability, weather splits + discipline, rate-based specialists, and all-time records). New client module `components/stats/circuits/{CircuitsSection,sections}.tsx` with circuit selector, scope + advanced filters (URL-synced under `c*` params), and drill-down links to `/schedule/[eventId]` and the Drivers tab. Denominator = starts (entries − DNS); averages/movement exclude DNF/DNS/DSQ; official (non-reverse) grid drives qualifying; records ignore active filters; no negative per-driver rankings. Added `circuitsTab.*` + circuit metric labels/tooltips (en + he) and tests (`tests/stats/circuit{Identity,Profile}.test.ts`). Removed the legacy in-file `CircuitsSection` and its exclusive helpers from `StatsPageContent.tsx`; the server page no longer computes/passes `circuits` (safety-car metrics are suppressed until the schedule column is actually populated, since the mapper coerces blanks to 0). | Bring the sparse new-league Circuits tab up to the Drivers/League bar: meaningful, transparent, filter-aware, i18n-first metrics with graceful small-sample degradation; kill the participation-grid + doc/data drift. |
