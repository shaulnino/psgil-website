...
- **Public accounts (PW-2b/2c): live with an approval workflow.** Self-service register/login/verify/profile at locale-prefixed routes (`/register`, `/login`, `/account`, `/verify`). New sign-ups are created **`status: "pending"`** (role `registered_user`, auto-signed-in into an "awaiting approval" state on `/account`); email verification is a separate gate. An **admin approves** them in the platform admin console (`/[locale]/admin`, gated by `requireAdmin`, reached from **My Account → Admin**): approve → `approved` + `driver` role by default (optional driver-link), reject, create-account, full-role editor, driver-link, suspend/reactivate, remove. The public site stays fully browsable for guests. Session/token/mailer live in `lib/auth/`. Input validated with `zod`. Account model has `status` + `driverId` + `emailVerified`; approval lifecycle in `lib/accounts/repository.ts` (`setAccountStatus`/`setDriverId`/`setAccountActive`).
...
> **Implementation status (2026-07-05):** PW-0, PW-1, PW-2a, PW-2b, **PW-2c** done. **Revised remaining phases:** PW-2d (steward↔driver-account integration: source steward participants from driver accounts, migrate `member`→`driver`) → PW-2e (driver profile & photo) → PW-2f (reset password) → PW-3 Attendance → PW-4 → PW-5.
