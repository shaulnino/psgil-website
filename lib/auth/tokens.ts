/**
 * Short-lived, purpose-scoped tokens for password reset (PW-2f). Signed with the
 * same HS256 session secret (PW-0 hard-fail in prod), so no new secret to manage.
 *
 * Single-use via `binding`: a caller can bind a token to a fingerprint of some
 * server-side state (e.g. the current password hash). Verification then requires
 * the fingerprint to still match, so once a reset changes the password hash the
 * link stops working — even before it expires, and even if replayed.
 */
import { createHash } from "node:crypto";
import { SignJWT, jwtVerify } from "jose";
import { getSessionSecret } from "@/lib/stewards/auth";

export type TokenPurpose = "reset-password";
