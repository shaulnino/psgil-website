...
> **⚠️ Superseded in part by PW-2g (2026-07-12) — admin-only account creation.** The **public self-registration, email-verification, and pending-approval** parts of this doc (§9 register/verify, §13 approval workflow, §14 pending state, and the "Registering is optional for a visitor" framing) **no longer reflect the product.** There is no `/register` or `/verify` route, no email verification, and no pending/approved/rejected `status` field. **Accounts are created only by an admin** in the platform admin console (`/[locale]/admin`); enable/disable is the single `isActive` flag. Password reset (PW-2f), the unified account model (PW-2a), driver-linking (PW-2c/d), and driver photo upload (PW-2e) all still stand. See ARCHITECTURE.md decision log (PW-2g) for the current model. The text below is retained as historical design intent.
>
...
- **PW-2c** — ✅ **Done.** Account `status` lifecycle (`pending`/`approved`/`rejected`; migrated users grandfathered `approved`). Registration now creates **pending** accounts (role `registered_user`, auto-login into an "awaiting approval" state on `/account`). New **platform admin console at `/[locale]/admin`** (admin-gated via `requireAdmin`), reached from the **My Account → Admin** menu entry: pending-approval queue (approve → `approved` + `driver` by default, optional driver-link / reject), create-account (provisioned approved, must-change-password), and an accounts table (full-role editor, driver-link select from the CSV roster, suspend/reactivate, remove). `driver` added to `view_steward_area` + participant permissions; `requireStewardUser` also blocks non-`approved` accounts. `/stewards/admin` **redirects to `/admin`**; steward nav entry removed. Verified in dev: register→pending banner; admin approve → approved+driver; redirect works; no errors. **Deferred:** admin editing another user's name/email or resetting their password (owners self-serve in `/account`); admin console body is English-only for now (localize later); `member`→`driver` data migration is PW-2d.
- **PW-2d** — ✅ **Done.** Steward participants are now **driver accounts**. Introduced `isDriverRole(roles)` (`driver` ∪ `member`) in `lib/accounts/types.ts` and replaced every `roles.includes("member")` participant check with it — the involved-drivers picker + complainant options + `createComplaintAction` validation (cases page/detail), the driver-response notification indicator, the dashboard "report incident" link, and the penalties driver dropdowns. `canCreateComplaint` now includes `driver`. **No data migration / no misclassification:** `member` is treated as a legacy alias for `driver` (so every pre-existing participant still works with zero regression), while new approved accounts get the `driver` role — realizing "driver count = driver-permission accounts" going forward. Legacy cases (reference account ids) unchanged. Verified: a `driver`-role account (no `member`) is a full steward participant; tsc clean. *(`member` fully retires as accounts are curated in the admin console; no risky blanket role rewrite.)*
- **PW-2e** — ✅ **Done.** Linked-driver **photo upload** in `/account`: stored dynamically (`lib/drivers/photoStore.ts` — per-driver Netlify Blob in prod, `public/uploads/drivers/{id}.{ext}` in dev), served in prod via `app/api/driver-photo/[driverId]`. The account stores the resolved `driverPhotoUrl` (with a `?v=` cache-buster). `applyUploadedDriverPhotos()` overrides the CSV `photo_url` on the drivers page (matched by `driverId → driver_id`), CSV as fallback. Upload gated to `isDriverRole` + linked (`driverId`); image-only, 5 MB cap. `isRemote()` in DriverCard/DriverModal broadened so query-string photos render `unoptimized` (Next 16 rejects local images with a query otherwise). Verified in dev: link → upload → drivers page shows the uploaded photo; tsc/lint clean, no errors.
  - **Update (2026-07-20):** the `/account` uploader now (a) offers an explicit **Replace** and **Remove** control once a photo exists (remove = `removeDriverPhotoAction` + `deleteDriverPhoto()`, reverts to the CSV `photo_url`/placeholder), and (b) runs the picked image through a **square (1:1) crop step** before upload (`react-easy-crop` + `cropImage.ts` canvas export → 512px JPEG), matching the square `object-cover` display on the drivers page. This is the first use of `react-easy-crop` in the repo.
- **PW-2f** — ✅ **Done (2026-07-12).** `/forgot-password` (anti-enumeration — identical response whether or not the account exists) + `/reset-password?token=…`. Reuses the PW-2b `reset-password` token; the token is **bound to a fingerprint of the current password hash** (`tokenBinding` in `lib/auth/tokens.ts`), so a link is single-use — rejected once the password changes, even within its 1h TTL. Login page surfaces the existing "Forgot your password?" link; reset redirects to `/login?reset=1` with a success banner. zod `forgotPasswordSchema`/`resetPasswordSchema`; en + he strings. Also aligned `lib/auth/mailer.ts` sender to `f1racingisl@gmail.com` (matching the league-email migration).
