...
- **shadcn go/no-go spike** (throwaway branch, not merged): run `shadcn init` against Tailwind v4 and confirm it does **not** write a `tailwind.config.js` and the build stays green. If it fails, resolve the init approach before Phase 3 (this is the design work's only P0 blocker).
  - **Spike result (2026-07-01): PASS, with a pinning caveat.** shadcn correctly detected Next.js + Tailwind v4 + the `@/` alias and wrote a `components.json` with `"tailwind": { "config": "" }` — i.e. the CSS-first v4 path, **no `tailwind.config.js` created**. The no-config invariant holds. Two caveats for Phase 3: (1) the *latest* CLI defaulted `style` to `base-nova` and emitted a newer schema (`menuColor`/`registries`), not the classic `new-york` — **set the style explicitly / pin the CLI** to a version offering `new-york`; (2) the latest CLI's dependency step invoked `deno` (`spawn deno ENOENT`) — **pin the CLI (it suggested `shadcn@4.11.0`) or install deps manually** (`clsx`, `tailwind-merge`, `class-variance-authority`, `lucide-react`) and add `components.json` + `cn()` by hand. Neither caveat is a blocker; the gating question (config file) is cleared.
